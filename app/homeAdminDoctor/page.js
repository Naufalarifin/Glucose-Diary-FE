...
import { Home } from '@/components/Managedoctor'
import axiosInstance from "@/lib/axios"

export default function HomeAdmin() {
    const [searchQuery, setSearchQuery] = useState('')
    const [isDashboardOpen, setIsDashboardOpen] = useState(true)
    const [activeSection, setActiveSection] = useState('manageDoctor')
          ...
          <div className="p-4 h-full flex flex-col">
            <nav className="flex-1">
              <ul className="space-y-2">
                <li>
                    <button
                        onClick={() => (handleSectionChange('manageDoctor'),handleHomeChange('Home'))}
                        className={`flex items-center w-full text-left ${
                        activeSection === 'manageDoctor' ? 'text-orange-500' : 'text-gray-600 hover:text-orange-500'
                        }`}
                    >
